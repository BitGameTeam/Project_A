...
public class GridManager : MonoBehaviour
{
            ...
            if (obstacleList !=null && obstacleList.Length > 0)
            {
                ...
                //foreach(GameObject data in obstacleList)
                {
                    Gizmos.DrawCube(GetGridCellCenter(
                        GetGridIndex(
                            //data.transform.position)), cellSize);
                            obstacleList[i].transform.position)), cellSize);
    ...
    private void DebugDrawGrid(Vector3 origin, 
        int numRows, int numCols, float cellSize, Color color)
    {
        ...
        {
            Vector3 startPos = origin + i * cellSize * new Vector3(0f, 1f, 0f);
        ...
        for (int i = 0; i < numCols + 1; ++i)
        {
            ...
            Vector3 endPos = startPos + height * new Vector3(0f, 1f, 0f);
            Debug.DrawLine(startPos, endPos, color);
        }
    }
}
