...
public class SkillManagement : MonoBehaviour
{
    ...
    public float Return_Skill(int skill_Number, Transform player)
    {
        ...
        switch (skill_Number)
        {
            ...
            case 4: StartCoroutine(ContinuousSlash()); break;
            case 5: StartCoroutine(WindSpin()); break;
        }
        return skill_cool;
    }
    ...
    //난도
    IEnumerator ContinuousSlash()
    {
        yield return new WaitForSeconds(0.4f);
        skill_List[4].SetActive(true);   
        StopCoroutine(ContinuousSlash());
    }
    //윈드스핀
    IEnumerator WindSpin()
    {
        yield return new WaitForSeconds(0.5f);
        skill_List[4].SetActive(true);
        StopCoroutine(HeavySlash());
